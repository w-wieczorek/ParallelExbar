...
def digits2chars(word):
    result = ""
    for digit in word:
        result += chr(ord(digit) + 49)
    return result
...
def readWords(file_name):
    ...
    with open(file_name, encoding='utf-8') as file:
        ...
        for _ in range(number_of_words):
            ...
            s = "".join(symbols)
            if cls == '1':
                positives.add(digits2chars(s))
            else:
                negatives.add(digits2chars(s))
...
def synthesize(X, Y, K):
    ...
    process = subprocess.run(['clingo', '-t', str(param_t), 'dfa.lp'], stdout=subprocess.PIPE)
    text = process.stdout
    text = text.decode()
    aut = None
...
parser.add_argument('--threads', help='use n threads', type=int, default=1)
